...
knitr::opts_chunk$set(engine.path=list(python='/usr/bin/python3'), echo=TRUE, results='asis', eval=TRUE)
...
for line in referencelengths:
  ...
  if int(linearray[1]) > 3500:
    baits_regions_mapped[linearray[0]] = {"reflength": int(linearray[1])}
...
```{python}
chromosomes=sorted(baits_regions_mapped.keys())
result = open("results/bait_mapped_to_solanum_filtered.txt")
for line in result:
  line == line.rstrip()
  if line == "": continue
  linearray=line.split()
  if linearray[1] in chromosomes and int(linearray[5]) == 0 and int(linearray[6]) == 1 and int(linearray[7]) == 120:  # these are number of gaps, start and end position of query
    #baits_regions_mapped[linearray[1]].update({'mapstart':int(linearray[8]), 'mapend': int(linearray[9])})
    ...
    baits_regions_mapped[linearray[1]]['mapStart']=[int(linearray[8])]
    baits_regions_mapped[linearray[1]]['mapEnd']=[int(linearray[9])]
result.close()
for key in list(baits_regions_mapped.keys())[:10]:
  print(key, baits_regions_mapped[key])
...
Now, using the bait regions mapped, lets get the random sequences
```{python}
sorted_chromosomes = sorted(baits_regions_mapped.keys())
total_number_of_chromosomes=len(sorted_chromosomes)
counter=0

for random_fragmentlength in fragment_lengths:
  found = False
  while not found:
    ...
    chromosome_length = baits_regions_mapped[chromosome_name]['reflength']
    
    #lets get random start position within the start and end mapping position
    random_startpos_in_bait_hitregion = np.random.randint(baits_regions_mapped[chromosome_name]['mapstart'], baits_regions_mapped[chromosome_name]['mapend'])
    ...
    if random_fragment_start < 1 and random-frament_end < chromosome_length - random_fragmentlength: 
      found = False
    else:
      baits_regions_mapped[chromosome_name].update({'start':random_fragment_start, 'end':random_fragment_end})
      found = True


for key in baits_regions_mapped.keys()[10]:
  print(key, baits_regions_mapped[key])
