...
for line in result:
  ...
  if line == "": continue
  linearray=line.split()
  if int(linearray[5]) == 0 and int(linearray[6]) == 1 and int(linearray[7]) == 120:  # these are number of gaps, start and end position of query
    bait_regions_mapped[linearray[1]].update({'mapstart':int(linearray[8]), 'mapend': int(linearray[9])})
result.close()
